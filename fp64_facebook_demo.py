...
def GM_DSPFP_per_float64(A1, A2, K=None, a=1, lamb=1, erro=0.1, 
    ...
    if K is not None:
        ...
        k = a_in**2
        K_in = (K / k).to(torch.float64)  
        A1_in = (A1 / a_in).to(torch.float64)  
        A2_in = (A2 / a_in).to(torch.float64)  
    else:
        K_in = torch.tensor(0, device=device, 
                            dtype=torch.float64)
        a1_in = torch.max(A1)
        a2_in = torch.max(A2)
        A1_in = (A1 / a1_in).to(torch.float64)  
        A2_in = (A2 / a2_in).to(torch.float64)  
    ...
    for m in range(num_iter):
        ...
        ind_per = []
        while ep >= erro and ind <= 30:

            result = A1_in @ (X @ A2_in) + lamb * K_in 

            Y[:n1, :n2] = result / torch.max(result)
            
            Y = (theta / 2) * Y
            
            Y, j = project_DS_per_float64(Y, n1, n2, 
                                          percent_in, 
                                          erro, maxit_in, n)            
            Y = Y[:n1, :n2]
...
# @profile
def project_DS_per_float64(X=None,
                        n1=None,
                        n2=None,
                        percent_in=None,
                        eps=None,
    ...
    # one1 = torch.ones(1, nn1, dtype=torch.float64, device=X.device)
    one2 = torch.ones((nn2, 1), dtype=torch.float64, device=X.device)
    X, i = while_loop_on_gpu(X, X_sum, n, one2, i)
    return X, i
    

@torch.jit.script
def while_loop_on_gpu(X, X_sum, n, one2, i):
    while (not X_sum):
...
def Evaluation(P,A1,A2,F1,F2):
    torch.backends.cuda.matmul.allow_tf32 = False
    dissimilarEdges = A1.float - P.float @ A2.float @ P.T.float
    dissimilarNodes = F1.float - P.float @ F2.float
...
profiler.add_function(GM_DSPFP_per_float64)  
profiler.add_function(project_DS_per_float64)  
...
for step in range(2):
    P, rtime  = GM_DSPFP_per_float64(adj_noisy25,adj_sorce, a=0.95, lamb=0, erro=0.1, theta=10)

print("Run time: "+str(rtime))
print("Node acc: "+str(sum(np.diag(P))/4039))
print("Edge acc: "+str(np.sum(np.multiply(adj_noisy25,P*adj_sorce*P.T))/adj_sorce.sum())) #Objective score/optimal score


profiler.runcall(GM_DSPFP_per_float64, adj_noisy25, adj_sorce, a=0.95, lamb=0, erro=0.1, theta=10)
